package com.rat.regresapitest.presentation.ui.global.commons.glide;
...
public class GlideBlurTransformation implements Transformation<Bitmap> {
    ...
    public Resource<Bitmap> transform(Resource<Bitmap> resource, int outWidth, int outHeight) {
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.JELLY_BEAN_MR2) {
            try {
                bitmap = renderScriptBlur(mContext, bitmap, radius);
            } catch (RSRuntimeException e) {
                bitmap = stackBlur(bitmap, radius, true);
            }
        } else {
            bitmap = stackBlur(bitmap, radius, true);
        }
    ...
    private Bitmap renderScriptBlur(Context context, Bitmap bitmap, int radius) throws RSRuntimeException {
    ...
    private Bitmap stackBlur(Bitmap sentBitmap, int radius, boolean canReuseInBitmap) {
